          ...
          if (product) {
            setProductData({
              ...
              sizes: product.sizes
                .replace(/[\[\]'""]/g, '')
                .split(',')
                .map((size) => size.trim()), // Removes quotes
            })
            setImage(product.image[0])
            setReviews(Math.floor(Math.random() * 200) + 50) // Generates reviews between 50-250
            setStars(Math.floor(Math.random() * 5) + 1) // Generates star rating between 1-5
          }
        }
      } catch (error) {
        console.error('Error fetching product details:', error)
